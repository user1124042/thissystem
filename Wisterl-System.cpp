...
void alpjsq() {
	...
	scanf("%s", &shuanshu[0]);
	if (strchr(shuanshu, '+') != NULL) {
		int left, right;
		try {
			char *lc = strchr(shuanshu, '+');
			right =
			atoi(strncpy(new char[lc - shuanshu + 1], shuanshu, lc - shuanshu));
			left = atoi(strncpy(new char[strlen(shuanshu) - (lc - shuanshu) + 1],
				lc + 1, strlen(shuanshu) - (lc - shuanshu)));
	...
	if (strchr(shuanshu, '^') != NULL) {
		...
		char *lc = strchr(shuanshu, '^');
		right = atoi(strncpy(new char[lc - shuanshu + 1], shuanshu, lc - shuanshu));
		left = atoi(strncpy(new char[strlen(shuanshu) - (lc - shuanshu) + 1],
			lc + 1, strlen(shuanshu) - (lc - shuanshu)));
		printf("%lld ^ %lld = %lld\n", right, left, cifang(right, left));
	}
	if (strchr(shuanshu, 's') != NULL) {
		printf("求根运算可能不准确\n");
		// 以下这些代码选自游戏《Quake-III Arena》
		char *lc = strchr(shuanshu, 's');
		double n = atof(strncpy(new char[strlen(shuanshu) - (lc - shuanshu) + 1],
			lc + 1, strlen(shuanshu) - (lc - shuanshu)));
		double j = n;
		if (n == 0)
			printf("sqrt(0) = 0\n");
...
void update() {
	...
	puts("2024-7-29 Wisterl ED 版本版本！！！又错了");
}
...
int main() {
	...
	if ((strcmp(qaz, "Ed_Wis_gfzh") == 0 && strcmp(wsx, "gh5ter") == 0) ||
		...
		puts("4399game···········4399");
		puts("cnjy·············网课");
		put("pblktct···········公告");puts("pblktct··········· 公告");
		put("sd·············苏打办公");puts("sd··············苏打办公");
		put("kkk··········西瓜视频");puts("kkk············· 西瓜视频");
		put("ybt··········信息学奥赛在线练习网站");puts("ybt············· 信息学奥赛在线练习网站");
		put("关于我们···········Wisterl公司介绍");puts("AboutWe············· Wisterl公司介绍");
		put("ys·········163邮箱");puts("ys·············  163邮箱");
		put("ajy··········爱奇艺");puts("ajy············· 爱奇艺");
		put("jjxrb·········狙击日本小");puts("jjxrb············ 狙击小日本");
		put("jd·············京东");puts("jd··············京东");
		put("qq··············腾讯网");puts("qq··············腾讯网");
		put("deplfy············深度翻译");puts("deplfy············ Deepl翻译");
		put("CPS··········CPS测试");puts("CPS············· CPS测试");
		put("ML··············未来信箱");puts("ML··············未来信箱");
		...
		虽然 （1 ） {
			...
			fflush (标准输出)；fflush(stdout);
			fflush (标准输入);fflush(stdin);
			如果（！找到）{if (!isfound) {
		...
		std::this_thread::sleep_for(std::chrono::seconds(1));sleep_for(std::chrono::seconds(1));
	} 别的 {} else {
